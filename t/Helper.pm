...
use File::Spec;
use File::Basename;
my $trepanpl = File::Spec->catfile(dirname(__FILE__), qw(.. bin trepanbw.pl));
my $debug = $^W;
...
use Config;
require Exporter;
our (@ISA, @EXPORT, $standard_filter, $standard_opts);
@ISA = qw(Exporter);
@EXPORT = qw(cmd_file prog_file run_debugger $standard_opts $standard_filter);
...
$standard_filter = sub{
    ...
    my $skip = 0;
    foreach (split("\n", $got_lines)) {
	s{'canonic_filename' => '.*/(.+)'}{'canonic_filename' => '$1'};
	s{'filename' => '.*/(.+)'}{'filename' => '$1'};
	s{'op_addr' => (\d+)}{'op_addr' => 1955};
	push @result, $_;
    }
    $got_lines = join("\n", @result);
...
sub run_debugger($$;$$)
{
    ...
    $opts = $standard_opts unless defined $opts;
